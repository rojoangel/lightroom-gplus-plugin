...
GPlusPublishServiceProvider.lua
Publish-specific portions of Lightroom GPlus uploader
...
--- The <i>service definition script</i> for a publish service provider associates 
 ...
 -- <p>The <code>GPlusPublishSupport.lua</code> file of the GPlus sample plug-in provides 
 ...
 -- 	<code>GPlusExportServiceProvider.lua</code> file.</p>
...
publishServiceProvider.small_icon = 'small_GPlus.png'
...
publishServiceProvider.titleForPublishedCollection = LOC "$$$/GPlus/TitleForPublishedCollection=Photoset"
...
publishServiceProvider.titleForPublishedCollection_standalone = LOC "$$$/GPlus/TitleForPublishedCollection/Standalone=Photoset"
...
-- publishServiceProvider.titleForPublishedCollectionSet = "(something)" -- not used for GPlus plug-in
...
--publishServiceProvider.titleForPublishedCollectionSet_standalone = "(something)" -- not used for GPlus plug-in
...
publishServiceProvider.titleForPublishedSmartCollection = LOC "$$$/GPlus/TitleForPublishedSmartCollection=Smart Photoset"
...
publishServiceProvider.titleForPublishedSmartCollection_standalone = LOC "$$$/GPlus/TitleForPublishedSmartCollection/Standalone=Smart Photoset"
...
function publishServiceProvider.getCollectionBehaviorInfo( publishSettings )
    ...
    return {
        defaultCollectionName = LOC "$$$/GPlus/DefaultCollectionName/Photostream=Photostream",
        defaultCollectionCanBeDeleted = false,
        canAddCollection = true,
        maxCollectionSetDepth = 0,
        -- Collection sets are not supported through the GPlus sample plug-in.
    }
...
publishServiceProvider.titleForGoToPublishedCollection = LOC "$$$/GPlus/TitleForGoToPublishedCollection=Show in GPlus"
...
publishServiceProvider.titleForGoToPublishedPhoto = LOC "$$$/GPlus/TitleForGoToPublishedCollection=Show in GPlus"
...
function publishServiceProvider.deletePhotosFromPublishedCollection( publishSettings, arrayOfPhotoIds, deletedCallback )
    ...
    for i, photoId in ipairs( arrayOfPhotoIds ) do
        ...
        GPlusAPI.deletePhoto( publishSettings, { photoId = photoId, suppressErrorCodes = { [ 1 ] = true } } )
        -- If GPlus says photo not found, ignore that.
...
function publishServiceProvider.metadataThatTriggersRepublish( publishSettings )
    ...
    return {
        ...
        -- also (not used by GPlus sample plug-in):
        -- customMetadata = true,
...
--[[ Not used for GPlus plug-in. This is an example of how this function might work.
...
-- the user-specified sort order should be followed as-is or reversed. The GPlus
-- sample plug-in uses this to reverse the order on the Photostream so that photos
-- appear in the GPlus web interface in the same sequence as they are shown in the
-- library grid.
-- <p>This is not a blocking call. It is called from within a task created
-- using the <a href="LrTasks.html"><code>LrTasks</code></a> namespace. In most
-- cases, you should not need to start your own task within this function.</p>
...
function publishServiceProvider.imposeSortOrderOnPublishedCollection( publishSettings, info, remoteIdSequence )
    ...
    if photosetId then
        ...
        local existingPhotoSequence = GPlusAPI.listPhotosFromPhotoset( publishSettings, { photosetId = photosetId } )
        ...
        if existingPhotoSequence and existingPhotoSequence.primary then
            GPlusAPI.setPhotosetSequence( publishSettings, {
...
-- publishServiceProvider.disableRenamePublishedCollection = true -- not used for GPlus sample plug-in
...
-- publishServiceProvider.disableRenamePublishedCollectionSet = true -- not used for GPlus sample plug-in
...
function publishServiceProvider.renamePublishedCollection( publishSettings, info )
    ...
    if info.remoteId then
        ...
        GPlusAPI.createOrUpdatePhotoset( publishSettings, {
...
function publishServiceProvider.deletePublishedCollection( publishSettings, info )
    ...
    import 'LrFunctionContext'.callWithContext( 'publishServiceProvider.deletePublishedCollection', function( context )
        ...
        local progressScope = LrDialogs.showModalProgressDialog {
            title = LOC( "$$$/GPlus/DeletingCollectionAndContents=Deleting photoset ^[^1^]", info.name ),
        ...
        if info and info.photoIds then
            ...
            for i, photoId in ipairs( info.photoIds ) do
                ...
                GPlusAPI.deletePhoto( publishSettings, { photoId = photoId } )
        ...
        if info and info.remoteId then
            ...
            GPlusAPI.deletePhotoset( publishSettings, {
                photosetId = info.remoteId,
                suppressError = true,
                -- GPlus has probably already deleted the photoset
...
function publishServiceProvider.getCommentsFromPublishedCollection( publishSettings, arrayOfPhotoInfo, commentCallback )
    ...
    for i, photoInfo in ipairs( arrayOfPhotoInfo ) do
        ...
        local comments = GPlusAPI.getComments( publishSettings, {
...
publishServiceProvider.titleForPhotoRating = LOC "$$$/GPlus/TitleForPhotoRating=Favorite Count"
...
function publishServiceProvider.getRatingsFromPublishedCollection( publishSettings, arrayOfPhotoInfo, ratingCallback )
    ...
    for i, photoInfo in ipairs( arrayOfPhotoInfo ) do
        ...
        local rating = GPlusAPI.getNumOfFavorites( publishSettings, { photoId = photoInfo.remoteId } )
...
function publishServiceProvider.canAddCommentsToService( publishSettings )
    ...
    return GPlusAPI.testGPlusConnection( publishSettings )
...
function publishServiceProvider.addCommentToPublishedPhoto( publishSettings, remotePhotoId, commentText )
    ...
    local success = GPlusAPI.addComment( publishSettings, {
...
GPlusPublishSupport = publishServiceProvider
