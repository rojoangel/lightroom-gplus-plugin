...
function GPlusAPI.showApiKeyDialog( message )
    ...
    LrErrors.throwUserError( LOC "$$$/GPlus/ApiKeyDialog/NoOauth=This sample plug-in was written for a deprecated form of Flickr authentication. Flickr now requires OAuth, which this plug-in does not support." )
    ...
    LrFunctionContext.callWithContext( 'GPlusAPI.showApiKeyDialog', function( context )
        ...
        local contents = f:column {
            ...
            f:static_text {
                title = LOC "$$$/GPlus/ApiKeyDialog/Message=In order to use this sample plug-in, you must obtain an API key from flickr.com. Sign on to Flickr and register for a key under Explore > Flickr Services > Your API Keys.",
                fill_horizontal = 1,
                width_in_chars = 55,
                height_in_lines = 2,
                size = 'small',
            ...
            f:row {
                ...
                f:static_text {
                    title = LOC "$$$/GPlus/ApiKeyDialog/Key=API Key:",
                    alignment = 'right',
            ...
            f:row {
                ...
                f:static_text {
                    title = LOC "$$$/GPlus/ApiKeyDialog/Secret=Shared Secret:",
                    alignment = 'right',
                    width = share 'title_width',
                },
        ...
        local result = LrDialogs.presentModalDialog {
            title = LOC "$$$/GPlus/ApiKeyDialog/Title=Enter Your Flickr API Key",
            contents = contents,
            accessoryView = f:push_button {
                title = LOC "$$$/GPlus/ApiKeyDialog/GoToFlickr=Get Flickr API Key...",
                action = function()
...
function GPlusAPI.getApiKeyAndSecret()
    ...
    type( apiKey ) == 'string' and #apiKey == 32 and
        ...
        if apiKey or sharedSecret then
            message = LOC "$$$/GPlus/ApiKeyDialog/Invalid=The key below is not valid."
        end

        GPlusAPI.showApiKeyDialog( message )
...
function GPlusAPI.makeApiSignature( params )

    -- If no API key, add it in now.

    local apiKey, sharedSecret = GPlusAPI.getApiKeyAndSecret()
...
function GPlusAPI.callRestMethod( propertyTable, params )

    -- Automatically add API key.

    local apiKey = GPlusAPI.getApiKeyAndSecret()
    ...
    params.api_sig = GPlusAPI.makeApiSignature( params )
    ...
    if simpleXml.stat == 'ok' or suppressError then
        ...
        LrErrors.throwUserError( LOC( "$$$/GPlus/Error/API=Flickr API returned an error message (function ^1, message ^2)",
...
function GPlusAPI.uploadPhoto( propertyTable, params )

    -- Prepare to upload.

    assert( type( params ) == 'table', 'GPlusAPI.uploadPhoto: params must be a table' )
    ...
    elseif params.photo_id and simpleXml.err and tonumber( simpleXml.err.code ) == 7 then
        ...
        return GPlusAPI.uploadPhoto( propertyTable, originalParams )

    else

        LrErrors.throwUserError( LOC( "$$$/GPlus/Error/API/Upload=Flickr API returned an error message (function upload, message ^1)",
            tostring( simpleXml.err and simpleXml.err.msg ) ) )
...
function GPlusAPI.openAuthUrl()

    -- Request the frob that we need for authentication.

    local data = GPlusAPI.callRestMethod( nil, { method = 'flickr.auth.getFrob', skipAuthToken = true } )
    ...
    local authApiSig = GPlusAPI.makeApiSignature{ perms = 'delete', frob = frob }
...
local function getPhotoInfo( propertyTable, params )
    ...
    if params.is_public == 1 then
        ...
        data, response = GPlusAPI.callRestMethod( nil, {
        ...
        data = GPlusAPI.callRestMethod( propertyTable, {
...
function GPlusAPI.constructPhotoURL( propertyTable, params )
...
function GPlusAPI.constructPhotosetURL( propertyTable, photosetId )
...
function GPlusAPI.constructPhotostreamURL( propertyTable )
...
function GPlusAPI.createOrUpdatePhotoset( propertyTable, params )
    ...
    if params.photosetId then
        ...
        data, response = GPlusAPI.callRestMethod( propertyTable, {
    ...
    if not needToCreatePhotoset then
        return params.photosetId, GPlusAPI.constructPhotosetURL( propertyTable, params.photosetId )
    else
...
function GPlusAPI.listPhotosFromPhotoset( propertyTable, params )
...
function GPlusAPI.addPhotosToSet( propertyTable, params )
...
function GPlusAPI.deletePhoto( propertyTable, params )
...
function GPlusAPI.deletePhotoset( propertyTable, params )
...
function GPlusAPI.setImageTags( propertyTable, params )
...
function GPlusAPI.getUserInfo( propertyTable, params )

    -- http://flickr.com/services/api/flickr.people.getInfo.html

    local data = GPlusAPI.callRestMethod( propertyTable, {
...
function GPlusAPI.getComments( propertyTable, params )
...
function GPlusAPI.addComment( propertyTable, params )
...
function GPlusAPI.getNumOfFavorites( propertyTable, params )
...
function GPlusAPI.testFlickrConnection( propertyTable )
