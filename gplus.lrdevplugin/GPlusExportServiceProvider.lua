...
--- The <i>service definition script</i> for an export service provider defines the hooks
 ...
 -- <p>The <code>GPlusExportServiceProvider.lua</code> provides examples for the hooks that
 --   a plug-in must provide in order to define an export service.
 --  Lightroom expects your plug-in to define the needed callbacks
...
for name, value in pairs( GPlusPublishSupport ) do
...
--------------------------------------------------------------------------------
--- (optional, Boolean)  When plug-in defined value is true, both video and
...
local displayNameForTitleChoice = {
    filename = LOC "$$$/GPlus/ExportDialog/Title/Filename=Filename",
    title = LOC "$$$/GPlus/ExportDialog/Title/Title=IPTC Title",
    empty = LOC "$$$/GPlus/ExportDialog/Title/Empty=Leave Blank",
}

local kSafetyTitles = {
    safe = LOC "$$$/GPlus/ExportDialog/Safety/Safe=Safe",
    moderate = LOC "$$$/GPlus/ExportDialog/Safety/Moderate=Moderate",
    restricted = LOC "$$$/GPlus/ExportDialog/Safety/Restricted=Restricted",
}
...
function exportServiceProvider.startDialog( propertyTable )
    ...
    require 'GPlusUser'
    GPlusUser.verifyLogin( propertyTable )
...

        {
            ...
            synopsis = function( props )
                if props.titleFirstChoice == 'title' then
                    return LOC( "$$$/GPlus/ExportDialog/Synopsis/TitleWithFallback=IPTC Title or ^1", displayNameForTitleChoice[ props.titleSecondChoice ] )
                else
            ...
            f:column {
                ...
                f:row {
                    ...
                    f:static_text {
                        title = LOC "$$$/GPlus/ExportDialog/ChooseTitleBySecondChoice=If Empty, Use:",
...
function exportServiceProvider.sectionsForBottomOfDialog( f, propertyTable )
    ...
    return {
        ...
        {
            title = LOC "$$$/GPlus/ExportDialog/PrivacyAndSafety=Privacy and Safety",
            synopsis = function( props )
                ...
                if props.privacy == 'private' then
                    add( LOC "$$$/GPlus/ExportDialog/Private=Private" )
                    if props.privacy_family then
                        add( LOC "$$$/GPlus/ExportDialog/Family=Family" )
                    end
                    if props.privacy_friends then
                        add( LOC "$$$/GPlus/ExportDialog/Friends=Friends" )
                    end
                else
                    add( LOC "$$$/GPlus/ExportDialog/Public=Public" )
                end
            ...
            f:column {
                ...
                f:row {
                    f:static_text {
                        title = LOC "$$$/GPlus/ExportDialog/Privacy=Privacy:",
                        alignment = 'right',
                        width = share 'labelWidth',
                    },

                    f:radio_button {
                        title = LOC "$$$/GPlus/ExportDialog/Private=Private",
                ...
                f:row {
                    ...
                    f:column {
                        ...
                        f:checkbox {
                            title = LOC "$$$/GPlus/ExportDialog/Family=Family",
                            value = bind 'privacy_family',
                            enabled = LrBinding.keyEquals( 'privacy', 'private' ),
                        },

                        f:checkbox {
                            title = LOC "$$$/GPlus/ExportDialog/Friends=Friends",
                ...
                f:row {
                    ...
                    f:radio_button {
                        title = LOC "$$$/GPlus/ExportDialog/Public=Public",
            ...
            f:column {
                ...
                f:row {
                    ...
                    f:checkbox {
                        title = LOC "$$$/GPlus/ExportDialog/HideFromPublicSite=Hide from public site areas",
                ...
                f:row {
                    ...
                    f:popup_menu {
                        ...
                        items = {
                            ...
                            { title = LOC "$$$/GPlus/ExportDialog/Type/Screenshot=Screenshot", value = 'screenshot' },
                            { title = LOC "$$$/GPlus/ExportDialog/Type/Other=Other", value = 'other' },
                        },
                    },
                },
            },
...
function exportServiceProvider.processRenderedPhotos( functionContext, exportContext )
    ...
    local photosetPhotoIds = photosetId and GPlusAPI.listPhotosFromPhotoset( exportSettings, { photosetId = photosetId } )
    ...
    if cannotRepublishCount	> 0 then
        ...
        local action = LrDialogs.promptForActionWithDoNotShow {
            ...
            verbBtns = {
                { label = LOC( "$$$/GPlus/Dialog/Buttons/FreeAccountErr/Skip=Skip" ), verb = "skip", },
                { label = LOC( "$$$/GPlus/Dialog/Buttons/FreeAccountErr/Replace=Replace" ), verb = "replace", },
            }
        }
        ...
        if not rendition.wasSkipped then
            ...
            if success then
                ...
                if didReplace then
                    ...
                    -- The replace call used by GPlusAPI.uploadPhoto ignores all of the metadata that is passed
                ...
                if not photosetUrl then
                    ...
                    if not isDefaultCollection then
                        ...
                        photosetId, photosetUrl = GPlusAPI.createOrUpdatePhotoset( exportSettings, {
                        ...
                        photosetUrl = GPlusAPI.constructPhotostreamURL( exportSettings )
